...
void display(void) {
    ...
    if (gameState == 0) if (gameState == 0 && parabens) {
    // Exibir a mensagem de vitória na tela inicial
    glColor3f(0.0, 0.0, 0.0);
    glRasterPos2f(205, 300);
    char *message = "Parabens, jogador! Voce venceu o jogo!";
    ...
    // Verificar se o jogo acabou devido a perda de vidas
    if (lives == 0) {
        gameover = 1;
    }
    
    // Exibir a mensagem de "Game over" na tela inicial
    if (gameState == 0 && gameover) {
        glColor3f(0.0, 0.0, 0.0);
        glRasterPos2f(205, 300);
        char *message = "Game over, jogador! Voce perdeu o jogo!";
        int len = strlen(message);
        int i;
        for (i = 0; i < len; i++) {
            glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, message[i]);
        }
        lives = 3; // Reiniciar vidas após exibir a mensagem "Game over"
    }
    // Reiniciar as vidas após exibir a mensagem de vitória ou "Game over"
    lives = 3;
}
else if (gameState == 1) {
