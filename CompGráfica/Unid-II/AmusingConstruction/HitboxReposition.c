...
void keyboard(int key, int x, int y) {
    if (gameState == 1) {
        ...
        if (x1 < size || x1 > screenWidth || y1 < size || y1 > screenHeight) {
            // Se houver colisão, fazer o quadrado voltar ao início
            x1 = 100;
            y1 = 100;
            collisionCount++; // Incrementar o contador de colisões apenas quando houver colisão
            if (collisionCount >= 3) {
                // Se o número de colisões exceder 3, encerrar o jogo
                gameState = 0;
                lives = 0; // Definir vidas como 0 para indicar game over
            } else {
                // Decrementar uma vida apenas se houver colisão e não exceder 3 colisões
                lives--;
                if (lives <= 0) {
        ...
        if ((y1 <= screenHeight / 2 + size && y1 >= screenHeight / 2 - size) || 
            (x1 <= screenWidth / 2 + size && x1 >= screenWidth / 2 - size)) {
        ...
        if (x1 <= size || x1 >= screenWidth - size || y1 <= size || y1 >= screenHeight - size) {
            // Se houver colisão, decrementar uma vida
            lives--;
            if (lives <= 0) {
                // Se as vidas acabarem, voltar para a tela inicial e reiniciar as vidas
