...
// Coordenadas e tamanho dos quadrados vermelhos
int redSquareCount = 4;
int redSquareX[4] = {200, 300, 400, 500};
int redSquareY[4] = {100, 200, 300, 400};
int redSquareSize = 20;
int redSquareSpeed = 2;
...
void display(void) {
    ...
    if (gameState == 0) {
        ...
        if (phaseTextStatus == 2) {
            moveRedSquares(); // Mover os quadrados vermelhos
            drawRedSquares(); // Desenhar os quadrados vermelhos
            checkCollisions(); // Verificar colisões com os quadrados vermelhos
        }
    ...
    static clock_t collisionTime = 0;
    if (collisionTime == 0) {
        collisionTime = clock();
    }
    ...
    // Se passaram mais de 2 segundos desde a colisão, voltar o quadrado para a posição inicial
    if (((double)(clock() - collisionTime) / CLOCKS_PER_SEC) > 2.0) {
        squareX = 100;
        squareY = 100;
...
void keyboard(int key, int x, int y) {
    if (gameState == 1) {
        ...
        // Verificar colisão com os quadrados vermelhos na fase 2
        if (phaseTextStatus == 2 && !isGhostMode) {
            for (int i = 0; i < 4; i++) {
                if (squareX + size >= redSquares[i].x && squareX <= redSquares[i].x + redSquares[i].size &&
                    ...
                    // Se houver colisão com um quadrado vermelho, perder uma vida
                    lives--;
                    if (lives <= 0) {
                        // Se as vidas acabarem, voltar para a tela inicial e reiniciar as vidas
                        gameState = 0;
...
void drawRedSquares(void) {
    for (int i = 0; i < 4; i++) {
        glColor3f(1.0, 0.0, 0.0); // Cor vermelha para os quadrados
        glBegin(GL_POLYGON);
...
void timer(int value) {
    if (isGhostMode) {
        ...
        // Desativar o modo fantasma após 5 segundos
        if (elapsedTime >= 5.0) {
            isGhostMode = 0;
            glutPostRedisplay();
        }
    }
    glutTimerFunc(1000 / 60, timer, 0); // Chama a função de timer a cada 1/60 de segundo
}
