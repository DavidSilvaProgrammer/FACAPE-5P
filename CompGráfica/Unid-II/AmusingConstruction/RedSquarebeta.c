...
// Variáveis para os quadrados vermelhos
RedSquare redSquares[5]; // Você pode ajustar o número de quadrados vermelhos conforme necessário
int numRedSquares = 0;
...
void display(void) {
    ...
    if (gameState == 0) {
        ...
        // Desenhar os quadrados vermelhos
        drawRedSquares();
        ...
        // Desenhar o quadrado verde
        drawGreenSquare();

        // Verificar colisão com os quadrados vermelhos
        checkCollisionWithRedSquares();
    }

    // Exibir o contador de tempo do modo fantasma e o cooldown do modo fantasma
    if (isGhostMode) {
        ...
        // Exibir o cooldown do modo fantasma
...
void drawRedSquares(void) {
    glColor3f(1.0, 0.0, 0.0); // Cor vermelha
    int i;
    for (i = 0; i < numRedSquares; i++) {
        glBegin(GL_POLYGON);
        glVertex2i(redSquares[i].x, redSquares[i].y);
        glVertex2i(redSquares[i].x + redSquares[i].size, redSquares[i].y);
        glVertex2i(redSquares[i].x + redSquares[i].size, redSquares[i].y + redSquares[i].size);
        glVertex2i(redSquares[i].x, redSquares[i].y + redSquares[i].size);
        glEnd();
    }
}

void updateRedSquares(void) {
    srand(time(NULL));
    int i;
    for (i = 0; i < numRedSquares; i++) {
        redSquares[i].x = rand() % (screenWidth - 50); // Randomiza a posição x dentro da janela
        redSquares[i].y = rand() % (screenHeight - 50); // Randomiza a posição y dentro da janela
        redSquares[i].size = 20; // Tamanho fixo para os quadrados vermelhos
    }
}

void checkCollisionWithRedSquares(void) {
	int i;
    for ( i = 0; i < numRedSquares; i++) {
        if (squareX + size >= redSquares[i].x && squareX <= redSquares[i].x + redSquares[i].size &&
            squareY + size >= redSquares[i].y && squareY <= redSquares[i].y + redSquares[i].size) {
            lives--; // Decrementa uma vida se houver colisão com um quadrado vermelho
            if (lives <= 0) {
                gameState = 0; // Se as vidas acabarem, volta para a tela inicial
                lives = 3;
            } else {
                squareX = 100; // Volta o quadrado cinza para a posição inicial
                squareY = 100;
            }
        }
    }
